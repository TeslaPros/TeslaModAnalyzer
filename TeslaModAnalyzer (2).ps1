...
# =========================
$doubleBufferProp = [System.Windows.Forms.Control].GetProperty(
    "DoubleBuffered",
...
function Enable-DoubleBuffer {
    ...
    if ($null -ne $Control -and $null -ne $doubleBufferProp) {
        [void]$doubleBufferProp.SetValue($Control, $true, $null)
    }
}
...
function New-Panel {
    param([int]$X, [int]$Y, [int]$W, [int]$H, [System.Drawing.Color]$Back = $Theme.Bg1)
...
function New-Label {
    param([string]$Text, [int]$X, [int]$Y, [int]$W, [int]$H,
          [System.Drawing.Color]$Fore = $Theme.Fore,
...
function New-Btn {
    param([string]$Text, [int]$X, [int]$Y, [int]$W, [int]$H,
          [System.Drawing.Color]$Back = $Theme.BtnGray,
          [System.Drawing.Color]$Fore = $Theme.Fore)
    ...
    $b.Add_MouseEnter({ $this.FlatAppearance.BorderColor = $Theme.AccentD })
    $b.Add_MouseLeave({ $this.FlatAppearance.BorderColor = $Theme.Border })
    return $b
}
...
function New-StatCard {
    ...
    $W = 290; $H = 100
    $card = New-Panel $X $Y $W $H $Theme.Bg2
    ...
    $lbl = New-Label $Label 18 12 240 20 $Theme.ForeD (fnt 9.5)
...
function New-Grid {
    ...
    $g.SetBounds($X, $Y, $W, $H)
    $g.BackgroundColor = $Theme.Bg1
    $g.BorderStyle = 'None'
    $g.GridColor = $Theme.Border
    $g.EnableHeadersVisualStyles = $false
    $g.ColumnHeadersDefaultCellStyle.BackColor = $Theme.Bg3
    $g.ColumnHeadersDefaultCellStyle.ForeColor = $Theme.Fore
    $g.ColumnHeadersDefaultCellStyle.Font = fntB 9.5
    $g.ColumnHeadersBorderStyle = 'Single'
    $g.ColumnHeadersHeight = 34
    $g.DefaultCellStyle.BackColor = $Theme.Bg1
    ...
    $g.DefaultCellStyle.SelectionForeColor = $Theme.White
    $g.AlternatingRowsDefaultCellStyle.BackColor = $Theme.Bg2
    $g.AlternatingRowsDefaultCellStyle.SelectionBackColor = $Theme.Selection
    $g.AlternatingRowsDefaultCellStyle.SelectionForeColor = $Theme.White
    ...
    $g.ScrollBars = 'Both'
    $g.DefaultCellStyle.WrapMode = 'False'
    $g.ColumnHeadersDefaultCellStyle.SelectionBackColor = $Theme.Bg3
    $g.ColumnHeadersDefaultCellStyle.SelectionForeColor = $Theme.Fore
...
function New-TextBox {
    ...
    $t.SetBounds($X, $Y, $W, $H)
    $t.BackColor = $Theme.Bg3
    $t.ForeColor = $Theme.Fore
...
$form.BackColor = $Theme.Bg0
$form.ForeColor = $Theme.Fore
$form.Font = fnt 10
Enable-DoubleBuffer $form

# --- Sidebar ---
$sidebar = New-Panel 0 0 220 1000 $Theme.Bg1
...
$logoLabel = New-Label "TESLA" 20 28 180 38 $Theme.Accent (fntB 26)
$proLabel  = New-Label "PRO" 100 30 80 32 $Theme.White (fntB 22)
$subLabel  = New-Label "Mod Analyzer V3" 22 68 180 20 $Theme.ForeD (fnt 9)
$sidebar.Controls.AddRange(@($logoLabel, $proLabel, $subLabel))

# Sidebar divider
$sideDiv = New-Panel 16 100 188 1 $Theme.Border
...
function New-NavBtn {
    ...
    $b.FlatAppearance.MouseOverBackColor = $Theme.Bg3
    $b.FlatAppearance.MouseDownBackColor = $Theme.Bg3
    $b.UseVisualStyleBackColor = $false
    $b.BackColor = $Theme.Bg1
    $b.ForeColor = $Theme.ForeD
    $b.Font = fnt 10.5
    $b.Cursor = [System.Windows.Forms.Cursors]::Hand
    $b.Add_MouseEnter({
        if ($this.Tag -ne "active") {
            $this.ForeColor = $Theme.Fore
        }
    })
    $b.Add_MouseLeave({
        if ($this.Tag -ne "active") {
            $this.ForeColor = $Theme.ForeD
...
$footerLabel = New-Label "Discord: @teamwsf" 20 940 180 20 $Theme.ForeD (fnt 9)
...
# --- Main host ---
$mainHost = New-Panel 220 0 1380 1000 $Theme.Bg0
...
function New-Page {
    param([string]$Name)
    $p = New-Panel 0 0 1380 1000 $Theme.Bg0
    $p.Visible = $false
...
function New-PageHeader {
    ...
    $hdr = New-Panel 20 18 1340 80 $Theme.Bg1
    ...
    $t = New-Label $Title 20 12 600 36 $Theme.Accent (fntB 22)
    $s = New-Label $Sub 22 50 700 18 $Theme.ForeD (fnt 9.5)
...
# Stat cards
$dashCard1 = New-StatCard "Verified Mods"    $Theme.Green  20  118
$dashCard2 = New-StatCard "Unknown Mods"     $Theme.Yellow 330 118
$dashCard3 = New-StatCard "Suspicious Mods"  $Theme.Red    640 118
$dashCard4 = New-StatCard "Scanned Files"    $Theme.Accent 950 118
...
# Quick actions panel
$dashActions = New-Panel 20 240 680 280 $Theme.Bg2
$dashActions.BorderStyle = 'FixedSingle'

$qTitle = New-Label "Quick Actions" 18 14 300 28 $Theme.Fore (fntB 14)
$btnDashScan    = New-Btn "▶  Start New Scan"      22 58  200 38 $Theme.BtnBlue $Theme.White
$btnDashResults = New-Btn "⊞  Open Results"        238 58 180 38
$btnDashExport  = New-Btn "↓  Export Results"      434 58 200 38

$summaryLbl  = New-Label "Latest Summary" 22 116 300 22 $Theme.Fore (fntB 11)
$dashSummary = New-TextBox 22 144 628 32 "" $true

$infoLbl = New-Label "Use Dashboard for a quick overview, Scan Center for a new scan, Results for detailed inspection." 22 195 630 50 $Theme.ForeD (fnt 9.5)

$dashActions.Controls.AddRange(@($qTitle,$btnDashScan,$btnDashResults,$btnDashExport,$summaryLbl,$dashSummary,$infoLbl))

# Recent history panel
$dashHistPanel = New-Panel 720 240 620 280 $Theme.Bg2
$dashHistPanel.BorderStyle = 'FixedSingle'

$dhTitle = New-Label "Recent History" 18 14 300 28 $Theme.Fore (fntB 14)
$dashHistList = New-Object System.Windows.Forms.ListBox
$dashHistList.SetBounds(16, 48, 588, 216)
$dashHistList.BackColor = $Theme.Bg3; $dashHistList.ForeColor = $Theme.Fore
...
# Path row
$scanPathPanel = New-Panel 20 118 1340 80 $Theme.Bg2
$scanPathPanel.BorderStyle = 'FixedSingle'

$pathLbl  = New-Label "Mods Folder" 18 10 120 18 $Theme.ForeD (fnt 9)
$pathBox  = New-TextBox 18 32 870 30 "$env:USERPROFILE\AppData\Roaming\.minecraft\mods"
$btnBrowse= New-Btn "Browse"  902 30 100 34
$btnScan  = New-Btn "▶ Scan"  1014 30 110 34 $Theme.BtnBlue $Theme.White
$btnClear = New-Btn "Clear"   1136 30 80 34  $Theme.RedD
...
# Options
$scanOptPanel = New-Panel 20 216 1340 180 $Theme.Bg2
$scanOptPanel.BorderStyle = 'FixedSingle'

$oTitle = New-Label "Scan Options" 18 12 300 26 $Theme.Fore (fntB 13)
...
function New-Chk {
    ...
    $c.Checked = $Checked
    $c.ForeColor = $Theme.Fore; $c.BackColor = [System.Drawing.Color]::Transparent
...
$scanHint = New-Label "Tip: keep the raw GitHub filename unchanged to allow CMD-based remote execution." 740 58 580 60 $Theme.ForeD (fnt 9)
...
# Progress panel
$scanProgPanel = New-Panel 20 414 1340 110 $Theme.Bg2
$scanProgPanel.BorderStyle = 'FixedSingle'

$progTitle  = New-Label "Scan Status" 18 10 300 24 $Theme.Fore (fntB 13)
...
$resCard1 = New-StatCard "Verified Mods"    $Theme.Green  20  118
$resCard2 = New-StatCard "Unknown Mods"     $Theme.Yellow 330 118
$resCard3 = New-StatCard "Suspicious Mods"  $Theme.Red    640 118
$resCard4 = New-StatCard "Scanned Files"    $Theme.Accent 950 118

$pageResults.Controls.AddRange(@($resCard1.Panel,$resCard2.Panel,$resCard3.Panel,$resCard4.Panel))

# Filter bar
$filterPanel = New-Panel 20 238 1340 56 $Theme.Bg2
$filterPanel.BorderStyle = 'FixedSingle'

$searchLbl = New-Label "Search" 14 18 58 20 $Theme.ForeD
$searchBox = New-TextBox 72 14 280 28

$catLbl = New-Label "Category" 368 18 72 20 $Theme.ForeD
$catCombo = New-Object System.Windows.Forms.ComboBox
$catCombo.SetBounds(440, 14, 160, 28)
$catCombo.DropDownStyle = 'DropDownList'; $catCombo.BackColor = $Theme.Bg3; $catCombo.ForeColor = $Theme.Fore
[void]$catCombo.Items.AddRange(@("All","Verified","Unknown","Suspicious"))
$catCombo.SelectedIndex = 0

$sortLbl = New-Label "Sort" 616 18 40 20 $Theme.ForeD
$sortCombo = New-Object System.Windows.Forms.ComboBox
$sortCombo.SetBounds(658, 14, 200, 28)
$sortCombo.DropDownStyle = 'DropDownList'; $sortCombo.BackColor = $Theme.Bg3; $sortCombo.ForeColor = $Theme.Fore
...
$resultsTabs.Appearance = 'Normal'
$resultsTabs.DrawMode = 'OwnerDrawFixed'
$resultsTabs.ItemSize = New-Object System.Drawing.Size(140, 28)
$resultsTabs.SizeMode = 'Fixed'
$resultsTabs.Add_DrawItem({
    param($sender, $e)
    $tabPage = $sender.TabPages[$e.Index]
    $isSelected = ($sender.SelectedIndex -eq $e.Index)
    $back = if ($isSelected) { $Theme.Bg3 } else { $Theme.Bg2 }
    $fore = if ($isSelected) { $Theme.Fore } else { $Theme.ForeD }
    $brush = [System.Drawing.SolidBrush]::new($back)
    $textBrush = [System.Drawing.SolidBrush]::new($fore)
    $sf = [System.Drawing.StringFormat]::new()
    $sf.Alignment = [System.Drawing.StringAlignment]::Center
    $sf.LineAlignment = [System.Drawing.StringAlignment]::Center
    $e.Graphics.FillRectangle($brush, $e.Bounds)
    $e.Graphics.DrawString($tabPage.Text.Trim(), $sender.Font, $textBrush, $e.Bounds, $sf)
    $brush.Dispose(); $textBrush.Dispose(); $sf.Dispose()
})

$tabV = New-Object System.Windows.Forms.TabPage; $tabV.Text = "  Verified  "; $tabV.BackColor = $Theme.Bg1
$tabU = New-Object System.Windows.Forms.TabPage; $tabU.Text = "  Unknown  "; $tabU.BackColor = $Theme.Bg1
$tabS = New-Object System.Windows.Forms.TabPage; $tabS.Text = "  Suspicious  "; $tabS.BackColor = $Theme.Bg1
...
# Details bar
$detailsBar = New-Panel 20 780 1340 100 $Theme.Bg2
$detailsBar.Anchor = 'Bottom,Left,Right'
$detailsBar.BorderStyle = 'FixedSingle'

$detLbl    = New-Label "Selected" 14 10 80 18 $Theme.ForeD (fntB 9)
...
$histPanel = New-Panel 20 118 1340 760 $Theme.Bg2
$histPanel.BorderStyle = 'FixedSingle'; $histPanel.Anchor = 'Top,Bottom,Left,Right'

$histTitle = New-Label "Saved scans" 18 14 300 24 $Theme.Fore (fntB 13)
$histList = New-Object System.Windows.Forms.ListBox
$histList.SetBounds(16, 46, 1308, 700)
$histList.BackColor = $Theme.Bg3; $histList.ForeColor = $Theme.Fore
...
$setPanel = New-Panel 20 118 800 400 $Theme.Bg2
$setPanel.BorderStyle = 'FixedSingle'

$setTitle = New-Label "General Settings" 18 14 300 26 $Theme.Fore (fntB 13)
...
$setNote = New-Label "Settings apply immediately where possible. Path memory applies within the current session." 22 206 740 40 $Theme.ForeD (fnt 9)
...
$statusBar.BackColor = $Theme.Bg1; $statusBar.SizingGrip = $false

$statusLbl = New-Object System.Windows.Forms.ToolStripStatusLabel
$statusLbl.Text = "Ready."; $statusLbl.ForeColor = $Theme.Fore; $statusLbl.Spring = $true
...
$exportDd.Text = "Export ▾"; $exportDd.ForeColor = $Theme.Fore
...
function Show-Page {
    ...
    foreach ($b in $navButtons) {
        $b.Tag = ""; $b.BackColor = $Theme.Bg1; $b.ForeColor = $Theme.ForeD
    }
    if ($map.ContainsKey($Name)) {
        $active = $map[$Name]
        $active.BackColor = $Theme.Bg3; $active.ForeColor = $Theme.Accent; $active.Tag = "active"
    }
}
...
function Apply-FilterAndSort {
    ...
    foreach ($grid in @($gridV,$gridU,$gridS)) {
        foreach ($col in $grid.Columns) {
            if ($col -is [System.Windows.Forms.DataGridViewColumn]) {
                $col.SortMode = 'Automatic'
            }
        }
    }
    if ($gridV.Columns.Count -gt 0) { $gridV.AutoResizeColumns('AllCells') }
    if ($gridU.Columns.Count -gt 0) { $gridU.AutoResizeColumns('AllCells') }
    if ($gridS.Columns.Count -gt 0) { $gridS.AutoResizeColumns('AllCells') }
...
function Add-HistoryEntry {
    ...
    $line = ('{0}  |  {1}  |  Files={2}  Verified={3}  Unknown={4}  Suspicious={5}' -f $ts, $Path, $Scanned, $Verified, $Unknown, $Suspicious)
...
function Export-Results {
    ...
    if ($Type -eq "TXT") {
        ...
        [void]$lines.Add("TESLAPRO PREMIUM MOD ANALYZER V3")
        [void]$lines.Add(("Path: {0}" -f $script:scanPath))
        [void]$lines.Add(("Scanned: {0}  Verified: {1}  Unknown: {2}  Suspicious: {3}" -f $script:totalFiles, $script:allVerified.Count, $script:allUnknown.Count, $script:allSuspicious.Count))
        [void]$lines.Add("")
        [void]$lines.Add("[ VERIFIED ]")
        foreach ($i in $script:allVerified) { [void]$lines.Add(("{0} | {1} | {2}" -f $i.ModName, $i.FileName, $i.Source)) }
        [void]$lines.Add("")
        [void]$lines.Add("[ UNKNOWN ]")
        foreach ($i in $script:allUnknown) { [void]$lines.Add(("{0} | {1}" -f $i.FileName, $i.ZoneId)) }
        [void]$lines.Add("")
        [void]$lines.Add("[ SUSPICIOUS ]")
        foreach ($i in $script:allSuspicious) { [void]$lines.Add(("{0} | {1} | {2}" -f $i.FileName, $i.DepFileName, ([string]::Join(', ',$i.StringsFound)))) }
